...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    shell: {
      target: {
        //command: 'open dist/macThesaurus.app'
    ...
    haml: {
      dist: {
        files: { 
          'app/views/index.html':'src/haml/index.haml'
        }
      }
    },
    coffee: {
      compile: {
        files : {
          'app/js/index.js':'src/coffeescript/index.coffee',
          'app/js/shortcut.js':'src/coffeescript/shortcut.coffee'
        }
      }
    },
    sass: {
    ...
    compress: {
      ...
      finalWindowsApp: {
        options: {
          archive: '<%= config.dist %>/macThesaurus.zip'
        },
        files: [{
          expand: true,
          cwd: '<%= config.tmp %>',
    ...
    rename: {
      app: {
        files: [{
          ...
          dest: '<%= config.dist %>/macThesaurus.app'
        }]
      },
      zipToApp: {
        files: [{
...
grunt.loadNpmTasks('grunt-bowercopy');
  ...
  //grunt.registerTask('default',['clean', 'coffee', 'haml', 'copy:appMacos:files', 'rename:app', 'chmod', 'shell']);
  grunt.registerTask('default',['clean', 'coffee', 'haml', 'sass', 'bowercopy', 'dist-mac', 'shell' ]);
  //grunt.loadNpmTasks('grunt-bower-requirejs');
  //grunt.registerTask('default', ['bower']);*/

  grunt.registerTask('chmod', 'Add lost Permissions.', function () {
    var fs = require('fs');
    fs.chmodSync('dist/macThesaurus.app/Contents/Frameworks/node-webkit Helper EH.app/Contents/MacOS/node-webkit Helper EH', '555');
    fs.chmodSync('dist/macThesaurus.app/Contents/Frameworks/node-webkit Helper NP.app/Contents/MacOS/node-webkit Helper NP', '555');
    fs.chmodSync('dist/macThesaurus.app/Contents/Frameworks/node-webkit Helper.app/Contents/MacOS/node-webkit Helper', '555');
    fs.chmodSync('dist/macThesaurus.app/Contents/MacOS/node-webkit', '555');
  });
...
grunt.registerTask('dmg', 'Create dmg from previously created app folder in dist.', function () {
  var done = this.async();
  var createDmgCommand = 'resources/mac/package.sh "macThesaurus"';
